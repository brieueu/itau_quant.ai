"""
Dashboard para o Modelo Híbrido de IA Quantitativa
Interface web moderna focada no modelo híbrido LSTM + Alfa
...
class GradioInterface:
    ...
    def enhanced_hybrid_wrapper(self, 
        tickers, vcov_period, vcov_window, alpha_period,
        benchmark, max_weight, min_weight, optimization_method, 
        allow_short, risk_free_rate
    ):
        """Wrapper aprimorado para o modelo híbrido com visualizações extras."""
        try:
            # Configurar restrições
            constraints = PortfolioConstraints(
                max_weight=float(max_weight) / 100,
            ...
            weights_fig = self._create_enhanced_hybrid_weights_chart(results)
            metrics_fig = self._create_enhanced_portfolio_metrics_chart(results)
            
            return weights_fig, metrics_fig, report
            
        except Exception as e:
            error_msg = f"❌ **Erro na otimização híbrida**: {str(e)}"
            empty_fig = go.Figure()
            return empty_fig, empty_fig, error_msg
    
    def _create_enhanced_hybrid_weights_chart(self, results):
        """Cria gráfico aprimorado de pesos otimizados do modelo híbrido."""
    ...
    def _create_enhanced_portfolio_metrics_chart(self, results):
        """Cria gráfico radar aprimorado com métricas do portfólio."""
        metrics = [
            'Sharpe Ratio',
            'Retorno Esperado (%)',
            'Diversificação',
    ...
    def create_interface(self):
        ...
        with gr.Blocks(
            ...
            <div style="text-align: center; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                ...
                <p><em>Inteligência artificial aplicada à otimização quantitativa de portfólios</em></p>
            </div>
            """)
            
            # Interface focada no modelo híbrido
            gr.Markdown("## 🧠 Modelo Híbrido V-Cov + Alfa")
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    hybrid_tickers_input = gr.Textbox(
                        label="Tickers do Portfólio",
                        value="AAPL, GOOGL, MSFT, AMZN, TSLA",
                        placeholder="Ex: AAPL, GOOGL, MSFT, AMZN",
                        info="Tickers separados por vírgula",
                        lines=2
                    )
                    
                    # Parâmetros LSTM V-Cov
                    gr.Markdown("### 🤖 Parâmetros LSTM")
                    with gr.Row():
                    ...
                    hybrid_optimization_method = gr.Radio(
                        label="Estratégia",
                        choices=[
                            ("📊 Máximo Sharpe", "max_sharpe"),
                            ("🛡️ Mínima Variância", "min_variance"), 
                            ("💰 Máximo Retorno", "max_return"),
                            ("⚖️ Risk Parity", "risk_parity"),
                ...
                with gr.Column(scale=2):
                    with gr.Tabs():
                        with gr.TabItem("🎯 Alocação"):
                            hybrid_weights_plot = gr.Plot(height=500)
                        
                        with gr.TabItem("📊 Métricas"):
                            hybrid_metrics_plot = gr.Plot(height=500)
                        
                        with gr.TabItem("📋 Relatório"):
                            hybrid_report_output = gr.Markdown(height=500)
            
            # Conectar eventos
            hybrid_btn.click(
                fn=self.enhanced_hybrid_wrapper,
                inputs=[
